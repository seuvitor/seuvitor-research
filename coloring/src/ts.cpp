...
void generateRandomSolution(Instance* instance, Solution* solution, char** countAdjColors, std::set<int>& conflictingVertices)
{
...
int calculateValue(Instance* instance, Solution* solution)
{
    int value = 0;
    for (int u = 0; u < instance->nvertices; ++u)
    {
...
void ts_constructSolution(Instance* instance, Solution* solution)
{
    ...
    generateRandomSolution(instance, &currentSolution, countAdjColors, conflictingVertices);
    int currentValue = calculateValue(instance, &currentSolution);
