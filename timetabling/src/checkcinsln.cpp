...
void getSolutionData(std::istream& in)
{
    for(int i = 0; i < noevents; i++)
    {
        in >> eventslots[i] >> eventrooms[i];
    }
}
...
{
    cout << "Couldn't open file " << slnfilename << ". Getting data from default input." << endl;
    getSolutionData(cin);
}
...
clog <<"Distance to feasibility = "<<distancetofeasibility<<endl;
clog <<"Total soft constraint penalty = "<<(longintensive+single+endofday)<<endl;
